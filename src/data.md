...
let aqi = await fetch(API_ENDPOINT).then((response) => {
  if (!response.ok) {
...
```js
aqi.forEach((a) => {
  a.timestamp = new Date(a.cst);
});
```

```js
let filteredData = view(
  Inputs.button("Click me", {
    ...
    reduce: () => {
      ...
      fetch(`${API_ENDPOINT}?pollutant=${pollutant.key}`)
        .then((res) => {
          if (!res.ok) {
      ...
      return aqi.filter(
        (t) => t.timestamp < endDate && t.timestamp >= startDate
      );
    },
  })
);
  ...
  display(
    resize((width) =>
      Plot.plot({
        ...
        width,
        x: { type: "utc", ticks: "minutes", label: null },
        y: { grid: true, inset: 10, label: "Air Quality Index" },
        marks: [
          Plot.dot(filteredData, {
            x: "timestamp",
            y: "particulate_matter_2_5",
            stroke: "rgba(75, 190, 190, 1)",
            fill: "rgba(75, 190, 190, 0.4)",
          }),
        ],
      })
    )
